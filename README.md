...
```
windows-wsl-optimizer/
├── setup.ps1       — 初期セットアップ（.wslconfig生成・Slack通知設定）
...
```powershell
# 基本セットアップ（.wslconfig生成 + Slack通知設定）
.\setup.ps1
...
`-Status` を付けると、.wslconfig・Slack環境変数の現在の設定を一覧表示します。
...
クリーンアップは手動で実行し、結果を Slack に通知します。
...
> **ヒント:** WSL2 の VHDX 圧縮（`-SkipWsl` なし）は WSL2 を一時停止するため、
> WSL を使っていないタイミングで管理者権限の PowerShell から実行してください。
